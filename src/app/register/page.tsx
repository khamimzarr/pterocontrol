...
export default async function RegisterPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="min-h-screen bg-[#05060f] flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 md:py-14 relative"><div className="w-full max-w-[420px] rounded-[16px] bg-[rgba(5,6,15,0.96)] border border-[rgba(186,215,247,0.12)] p-6 md:p-7 shadow-[inset_0_1px_1px_rgba(216,236,248,0.20),0_24px_48px_rgba(0,0,0,0.45)]"><div className="flex items-center gap-2"><span className="w-8 h-8 rounded-full grid place-items-center bg-[#663af3] text-white font-medium text-[13px] shadow-[0_0_16px_rgba(102,58,243,0.45)]">◈</span><span className="text-[11px] tracking-[0.08em] uppercase font-[var(--font-dotdigital)] text-[#9da7ba]">Daftar</span></div><h1 className="mt-3 font-[var(--font-aeonikpro)] font-medium text-[22px] leading-none text-white">Buat akun.</h1><p className="mt-2 text-[13px] text-[#9da7ba]"><span className="text-[#d1e4fa] font-medium">PENDING</span> sampai admin setujui.</p>{err && <div className="mt-4 rounded-[10px] bg-[rgba(228,109,76,0.10)] border border-[rgba(228,109,76,0.22)] px-4 py-2.5 text-[13px] text-[#e46d4c]">{err === "invalid" ? "Email/sandi tidak valid. Min 8 karakter." : err}</div>}<form action={register} className="mt-5 space-y-3.5"><label className="block"><span className="text-[11px] tracking-[0.06em] uppercase font-[var(--font-dotdigital)] text-[#c7d3ea]">Email</span><input name="email" type="email" required autoComplete="email" placeholder="kamu@email.com" className="auth-input mt-1.5 w-full px-3 py-2.5 text-[14px]" /></label><label className="block"><span className="text-[11px] tracking-[0.06em] uppercase font-[var(--font-dotdigital)] text-[#c7d3ea]">Kata sandi</span><input name="password" type="password" required autoComplete="new-password" placeholder="Min 8 karakter" className="auth-input mt-1.5 w-full px-3 py-2.5 text-[14px]" /></label><button type="submit" className="w-full mt-1 flash-violet rounded-[6px] py-2.5 text-[14px] font-medium text-white">Continue</button></form><p className="mt-4 text-center text-[12px] text-[#9da7ba]">Punya akun? <Link href="/login" className="text-[#d1e4fa] hover:text-white hover:underline">Masuk →</Link></p></div></div>
