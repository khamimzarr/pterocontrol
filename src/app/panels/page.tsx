  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-[16px] bg-[rgba(5,6,15,0.65)] border-b border-[rgba(186,215,247,0.08)]">
      <div className="mx-auto max-w-[1200px] px-6 h-[52px] flex items-center justify-between">
        <Link href="/dashboard" className="flex items-center gap-2.5"><span className="w-7 h-7 rounded-full grid place-items-center bg-[rgba(186,214,247,0.08)] border border-[rgba(186,215,247,0.12)]"><span className="w-2.5 h-2.5 rounded-full bg-[#663af3] animate-shimmer-dot" /></span><span className="font-medium text-[15px] text-[#d1e4fa]">PteroControl</span><span className="hidden sm:inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium text-[#c7d3ea] bg-[rgba(199,211,234,0.10)] border border-[rgba(186,215,247,0.06)]">panel</span></Link>
        ...
        <div className="mx-auto max-w-[1200px] px-6 md:px-10 space-y-4">
          <div className="rounded-[16px] bg-[rgba(5,6,15,0.96)] border border-[rgba(186,215,247,0.12)] p-6 shadow-[inset_0_1px_1px_rgba(216,236,248,0.20),0_24px_48px_rgba(0,0,0,0.45)]">
            ...
            <form action={addPanel} className="mt-5 grid md:grid-cols-3 gap-3">
              <label className="block"><span className="text-[11px] tracking-[0.06em] uppercase font-[var(--font-dotdigital)] text-[#c7d3ea]">Nama</span><input name="panelName" required maxLength={100} placeholder="eu-1" className="auth-input mt-1.5 w-full px-3 py-2.5 text-[13px]" /></label>
              <label className="block"><span className="text-[11px] tracking-[0.06em] uppercase font-[var(--font-dotdigital)] text-[#c7d3ea]">URL</span><input name="panelUrl" type="url" required placeholder="https://panel.contoh.com" className="auth-input mt-1.5 w-full px-3 py-2.5 text-[13px]" /></label>
              <label className="block"><span className="text-[11px] tracking-[0.06em] uppercase font-[var(--font-dotdigital)] text-[#c7d3ea]">API key</span><input name="apiKey" type="password" required minLength={5} placeholder="ptlc_••••" className="auth-input mt-1.5 w-full px-3 py-2.5 text-[13px]" /></label>
              <div className="md:col-span-3 flex items-center gap-3 pt-1"><button type="submit" className="flash-violet rounded-[6px] px-5 py-2.5 text-[13px] font-medium text-white">Continue</button><span className="text-[11px] text-[#9da7ba]">AES-256</span></div>
            </form>
          </div>
          {count === 0 ? (
              ...
              {panels!.map((p) => (
                <div key={p.id} className="glass-card rounded-[16px] p-5">
                  ...
                  <details className="mt-3 group"><summary className="text-[13px] text-[#b6d9fc] cursor-pointer hover:text-white list-none inline-flex items-center gap-1">Edit <span className="group-open:rotate-90 transition-transform">›</span></summary>
                    <form action={editPanel} className="mt-3 grid md:grid-cols-3 gap-3 border-t border-[rgba(186,215,247,0.08)] pt-3">
                      <input type="hidden" name="id" value={p.id} /><input name="panelName" required defaultValue={p.panel_name} className="auth-input px-3 py-2.5 text-[13px]" /><input name="panelUrl" type="url" required defaultValue={p.panel_url} className="auth-input px-3 py-2.5 text-[13px]" /><input name="apiKey" type="password" required minLength={5} placeholder="ptlc_ baru" className="auth-input px-3 py-2.5 text-[13px]" />
